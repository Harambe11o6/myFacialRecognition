...
class LoginPage:
    def __init__(self, window):
        ...
        # ============================Forgot password=============================
        # ========================================================================
        self.forgot_button = Button(self.lgn_frame, text="Forgot Password ?",command=ForgotPasswd,
...
class ForgotPasswordPage:
    def __init__(self, window):
        ...
        # ============================username====================================
        # ========================================================================
        self.username_label = Label(self.lgn_frame, text="Registered Username", bg="#040405", fg="#4f4e4d",
                                    font=("yu gothic ui", 13, "bold"))
        self.username_label.place(x=550, y=300)

        self.username_entry = Entry(self.lgn_frame, textvariable=check_name, highlightthickness=0, relief=FLAT,
                                    bg="#040405", fg="#6b6a69",
                                    font=("yu gothic ui ", 12, "bold"), insertbackground='#6b6a69')
        self.username_entry.place(x=580, y=335, width=270)
        ...
        self.lgn_button_label.image = photo
        self.lgn_button_label.place(x=550, y=450)
        self.login = Button(self.lgn_button_label, command=login, text='SAVE NEW PASSWORD', font=("yu gothic ui", 13, "bold"),
                            width=25, bd=0,
                            bg='#3047ff', cursor='hand2', activebackground='#3047ff', fg='white')
        self.login.place(x=20, y=10)
        # ========================================================================
        # ============================New password====================================
        # ========================================================================
        self.password_label = Label(self.lgn_frame, text="New Password", bg="#040405", fg="#4f4e4d",
                                    font=("yu gothic ui", 13, "bold"))
        self.password_label.place(x=550, y=380)

        self.password_entry = Entry(self.lgn_frame, highlightthickness=0, textvariable=corrected_password, relief=FLAT,
                                    bg="#040405", fg="#6b6a69",
                                    font=("yu gothic ui", 12, "bold"), show="*", insertbackground='#6b6a69')
        self.password_entry.place(x=580, y=416, width=244)
...
def regwrite():#validating registration details given by the user
    ...
    seccon.close()
def ForgotPasswd():
    global forgotpassword_window
    forgotpassword_window = Tk()
    global check_name
    global corrected_password
    check_name=StringVar()
    corrected_password=StringVar()
    ForgotPasswordPage(forgotpassword_window)
    ForgotPasswordPage.mainloop()
def correction():
    mycon=mysql.connector.connect(host="localhost ", user="root", passwd="SPINteg@123", database="sample")
    cursor2=mycon.cursor()
    cursor2.execute("select name from details")
    names_list=cursor2.fetchall()
    if(check_name not in names_list):
        messagebox.showinfo("Login Status", "Username doesnt match with the database")
    else:
        cursor2.execute("update detail set DOB='{}' where name='{}'".format(corrected_password,check_name))
    mycon.commit()
    mycon.close()
